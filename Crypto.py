...
def log_to_field(text):
    """Prints the text to the log field."""
    progress_field.insert(END, text + "\n")
    root.update()


def make_line():
    """Prints the line separator to the log field."""
    log_to_field("-" * 40)


def choose_directory():
    """Dialog of getting the directory to encrypt/decrypt."""
...
def crypt():
    """Calls encrypt/decrypt function depending on the option a user have chosen."""
...
def generate_password():
    """Generates a random password and inserts it to the password entry field."""
...
copyButton = Button(main_frame, text="Copy", font=default_font, relief=GROOVE, command=copy_text)
...
copyButton.grid(row=6, column=1, sticky=W, padx=10, pady=5)
...
app_menu.add_command(label="Exit", command=app_exit)
help_menu = Menu(main_menu, tearoff=0)
help_menu.add_command(label="Our website",
                      command=lambda: webbrowser.open("https://anideasoft.blogspot.com"))
help_menu.add_command(label="Our GitHub",
                      command=lambda: webbrowser.open("https://github.com/AlanTheKnight/crypto"))
help_menu.add_command(label="Instructions",
                      command=lambda: webbrowser.open("https://anideasoft.blogspot.com/p/crypto-instructions.html"))
main_menu.add_cascade(label="CRYPTO", menu=app_menu)
